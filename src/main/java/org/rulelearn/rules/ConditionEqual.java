 ...
 */
public abstract class ConditionEqual<T extends EvaluationField> extends Condition<T> {
	 ...
	 */
	ConditionEqual(EvaluationAttributeWithContext attributeWithContext, T limitingEvaluation) {
	...
	public RuleSemantics getRuleSemantics() {
		if (this.attributeWithContext.getAttributeType() != AttributeType.DECISION) {
			throw new InvalidValueException("Cannot establish rule semantics given an equality condition not defined on a decision attribute.");
		}
		
		if (this.attributeWithContext.getAttributePreferenceType() == AttributePreferenceType.NONE) {
			return RuleSemantics.EQUAL;
		} else {
			throw new InvalidValueException("Cannot establish rule semantics given an equality condition w.r.t. a decision attribute having preference type.");
