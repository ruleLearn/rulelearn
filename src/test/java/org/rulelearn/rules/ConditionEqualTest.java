 ...
 */
class ConditionEqualTest {

	/**
	 * Mock of {@link ConditionEqual}, used to provide an ad hoc implementation of abstract methods of that class.
	 * Each invocation of an abstract method on {@link #condition} results in invocation of that method on {@link #conditionMock}.
	 * In other words, {@link #condition} implements each abstract method of {@link ConditionEqual} by calling respective method on
	 * {@link #conditionMock}.
	 */
	private ConditionEqual<? extends EvaluationField> conditionMock;
	...
	private static <T extends EvaluationField> boolean satisfiedByHelper(ConditionEqual<T> conditionMock, EvaluationField evaluation) { //applies wildcard capture
	...
	private void setup() {
		...
		Mockito.when(this.attributeWithContextMock1.getAttributePreferenceType()).thenReturn(AttributePreferenceType.NONE);
		...
		Mockito.when(this.attributeWithContextMock2.getAttributePreferenceType()).thenReturn(AttributePreferenceType.NONE);
		
		this.conditionMock = Mockito.mock(ConditionEqual.class);
	}
	
	private <T extends EvaluationField> ConditionEqual<T> createCondition(EvaluationAttributeWithContext attributeWithContext, T limitingEvaluation) {
		return new ConditionEqual<T>(attributeWithContext, limitingEvaluation) {
			@Override
			...
			@Override
			@SuppressWarnings("unchecked")
			public ConditionEqual<T> duplicate() {
				return (ConditionEqual<T>)conditionMock.duplicate(); //TODO: test this cast
			}
	...
	/**
	 * Test method for {@link ConditionEqual#ConditionEqual(EvaluationAttributeWithContext, EvaluationField)}.
	 */
	@Test
	void testConditionEqual() {
		try {
			createCondition(null, limitingEvaluation1);
			fail("Should not create equality condition with null evaluation attribute with context.");
		...
		try {
			...
			fail("Should not create equality condition with null limiting evaluation.");
		} catch (NullPointerException exception) {
			//do nothing => exception is correctly thrown
		}
	}	
	...
	/**
	 * Test method for {@link ConditionEqual#getRelationSymbol()}.
	 */
	@Test
	...
	/**
	 * Test method for {@link ConditionEqual#getRuleSemantics()}.
	 */
	@Test
	void testGetRuleSemantics01() {
		assertEquals(createCondition(attributeWithContextMock1, limitingEvaluation1).getRuleSemantics(), RuleSemantics.EQUAL);
	}
	...
	void testGetRuleSemantics02() {
		assertEquals(createCondition(attributeWithContextMock2, limitingEvaluation2).getRuleSemantics(), RuleSemantics.EQUAL);
	}
